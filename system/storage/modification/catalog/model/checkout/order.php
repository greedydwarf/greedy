	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'total_usd'               => $order_query->row['total_usd'],
				...
				'date_pay'                => $order_query->row['date_pay']
			);
		} else {
			return false;
		}
      ...
      //Shoputils.Mail.Order.Status End
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				if ($order_info['language_id'] == 1){
					$subject = $this->config->get('config_mail_theme1');
				} else {
					$subject = $this->config->get('config_mail_theme2');
				}
				...
				if ($order_info['language_id'] == 1){
					$format = $this->config->get('config_mail_contentmail1');
				} else {
					$format = $this->config->get('config_mail_contentmail2');
				}
				...
				if ($order_info['language_id'] == 1){
					$total = $order_products[0]['total'];
				} else {
					$total = $order_products[0]['total_usd'];
				}
				...
				$replace = array(
					'nick_name' => $order_info['firstname'],
					'server'  => $order_info['lastname'],
					'payment_num'   => $order_info['payment_address_1'],
					'payment_method' => $order_info['payment_method'],
					'date_pay' => $order_info['date_pay'],
					'product_name'      => $order_products[0]['name'],
					'game_currency'  => $order_products[0]['model'],
				...
				$custom_mail = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));

				$theme = str_replace($find, $replace, $subject);
				...
				$mail->setHtml(html_entity_decode($custom_mail, ENT_QUOTES, 'UTF-8'));
